...
export default function RequestPage() {
  ...
  const handleEditClick = (req) => {
    ...
    const mode = req.documentType === "Custom Request" ? "custom" : "document_type";
    setFormData({
      creationMode: mode,
      documentType: mode === "custom" ? "" : req.documentType,
  ...
  const saveRequest = async () => {
    if (formData.creationMode === "document_type" && !formData.documentType) {
      showAlert("Please select a Document Type.");
      return;
    }
    if (formData.creationMode === "custom" && (!formData.fromDepartment || !formData.toDepartment)) {
    ...
    const senderDept = formData.creationMode === "custom" ? formData.fromDepartment : (currentUser?.department || currentUser?.mainRole || "Unknown");
    const initialStepIndex = 0; // Always start at step 0 so the department manager must approve it
    let actualTargetDept = "global";
    let stepsCount = 3;
    let pathSteps = [];

    if (formData.creationMode === "document_type") {
      actualTargetDept = selectedDocType?.steps?.[initialStepIndex]?.department || selectedDocType?.steps?.[initialStepIndex]?.mainRole || "global";
      stepsCount = selectedDocType?.totalSteps || 3;
      pathSteps = selectedDocType?.steps || [];
    } else {
      actualTargetDept = formData.toDepartment;
      stepsCount = 1;
      pathSteps = [
        {
    ...
    if (selectedRequest && viewState === "CREATE") {
      const updatedReq = {
        ...selectedRequest,
        ...formData,
        documentType: formData.creationMode === "custom" ? "Custom Request" : formData.documentType,
        files: filesToSave,
        path: pathSteps,
        stepsCount: stepsCount,
      };
      updatedList = requests.map(r => r.id === selectedRequest.id ? updatedReq : r);
      try {
    ...
    try {
      ...
      window.dispatchEvent(new Event("requests_updated"));
      setRequests(updatedList);
      setFormData({ creationMode: "custom", documentType: "", fromDepartment: "", toDepartment: "", title: "", urgency: "", description: "" });
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-gray-50 dark:bg-[#242B36] border-b border-gray-200 dark:border-[#2A2F3A]">
                        <tr>
                          ...
                          <th className="py-4 px-4 font-bold text-black dark:text-white text-[15px]">From Department</th>
                          <th className="py-4 px-4 font-bold text-black dark:text-white text-[15px]">To Department</th>
                            ...
                            return (
                              <tr key={req.id} className={`hover:bg-gray-50/50 dark:hover:bg-[#242B36] transition-colors ${isSelected ? 'bg-green-50/30' : ''}`}>
                                ...
                                <td className="py-4 px-4   text-[15px]">
                                  {(() => {
                                    const p = (req.urgency || "Normal").toLowerCase().trim();
                                    if (p === "urgent") return (
                                    ...
                                    return (
                                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400 font-extrabold text-[10px] uppercase tracking-wider border border-blue-200 dark:border-blue-500/20 shadow-sm">
                                ...
                                <td className="py-4 px-4">
                                  {req.status === "In Progressing" ? (
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#fef9c3] text-[#a16207] dark:bg-[#a16207]/10 dark:text-[#fde047] font-extrabold text-[10px] uppercase tracking-wider border border-[#fef08a] dark:border-[#ca8a04]/30 shadow-sm">
                  ...
                  <div className="w-full xl:w-[30%] flex flex-col gap-6">
                    <div className="bg-white dark:bg-[#161B22] border border-gray-200 dark:border-[#2A2F3A] rounded-lg p-6 shadow-sm flex flex-col gap-4 sticky top-6">
                      ...
                      <div className="flex flex-col gap-3">
                        <label className="block text-[13px] font-medium text-gray-600 dark:text-[#a1a1aa]">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-[#2A2F3A] focus:border-[#1a5b28] outline-none text-[14px] bg-gray-50 dark:bg-[#242B36]/50 transition-colors"
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {["Normal", "High", "Urgent"].map(opt => (
                            <button
                              ...
                              className={`px-3 py-1.5 text-[12px] font-bold rounded-md border transition-colors flex-1 text-center ${formData.urgency === opt
